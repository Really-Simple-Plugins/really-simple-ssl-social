...
<table class="form-table rsssl-license-table">
	...
	if ($status=='valid' || $license_data->license=='site_inactive') {
		...
		if ($license_data->activations_left < $license_data->license_limit) {
			$rsssl_soc_licensing->rsssl_notice(sprintf(__('You have %d activations left on your license. If you need more activations you can upgrade your current license to %s on your %saccount%s page.', "'really-simple-ssl-soc'"), $license_data->activations_left, $upgrade, '<a href="https://really-simple-ssl.com/account" target="_blank">', '</a>'), 'open', __('Open', "'really-simple-ssl-soc'"));
		}
	}

	if ($message) {
		$rsssl_soc_licensing->rsssl_notice($message,'warning', __('Warning', "'really-simple-ssl-soc'"));
	} elseif ($license_data->license == 'deactivated'){
		if ($status=='valid'){
			$rsssl_soc_licensing->rsssl_notice(__("Your license is valid, but not activated on this site", "'really-simple-ssl-soc'"), 'open', __('Open', "'really-simple-ssl-soc'"));
		} elseif(!empty($status)) {
			$rsssl_soc_licensing->rsssl_notice(__("Your license does not seem to be valid. Please check your license key", "'really-simple-ssl-soc'"), 'open', __('Open', "'really-simple-ssl-soc'"));
		}
	} elseif ($status == 'valid') {
		$date = $license_data->expires;
		$date = strtotime($date);
		$date = date(get_option('date_format'), $date);
		$rsssl_soc_licensing->rsssl_notice(sprintf(__("Your license is valid, and expires on: %s", "'really-simple-ssl-soc'"), $date), 'success', __('Success', "'really-simple-ssl-soc'"));
	} elseif ($license_data->license == 'expired') {
		$link = '<a target="_blank" href="' . $rsssl_soc_licensing->website . "/account/" . '">';
		$rsssl_soc_licensing->rsssl_notice(sprintf(__("Your license key has expired. Please renew your license key on %syour account page%s", "'really-simple-ssl-soc'"), $link, '</a>'), 'warning', __('Warning', "'really-simple-ssl-soc'"));
		...
		$rsssl_soc_licensing->rsssl_notice(__("Enter your license here so you keep receiving updates and support.", "'really-simple-ssl-soc'"), 'open', __('Open', "'really-simple-ssl-soc'"));
	}
	?>
	</tbody>
</table>
